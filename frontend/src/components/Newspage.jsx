  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (storedData) {
          storedDataInJson = JSON.parse(storedData).feed; // Updated to use "feed"
        }
        ...
        if (storedData && todayDate === storedDate) {
          ...
          console.log("Stored data exists but date has changed. Fetching new data...");
          localStorage.removeItem("newsData");
          localStorage.removeItem("newsDataDate");
        }
    
        const response = await fetch("http://localhost:3001/alpha", {
          method: "GET",
          headers: {
        ...
        if (response.ok) {
          ...
          setNewsData(data.feed); // Updated to use "feed"
